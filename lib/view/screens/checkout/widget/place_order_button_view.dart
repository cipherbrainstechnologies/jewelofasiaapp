...
class PlaceOrderButtonView extends StatelessWidget {
  ...
  const PlaceOrderButtonView({Key? key, this.fromOfflinePayment = false})
      : super(key: key);
                ...
                child: Column(children: [
                  if (!fromOfflinePayment)
                    ResponsiveHelper.isDesktop(context)
                        ...
                        : TotalAmountView(
                            ...
                            freeDelivery: CheckOutHelper.isFreeDeliveryCharge(
                  ...
                  CustomButton(
                    ...
                    buttonText: getTranslated(
                        fromOfflinePayment ? 'submit' : 'place_order', context),
                    onPressed: () {
                      if (fromOfflinePayment) {
                      ...
                      if (fromOfflinePayment &&
                          orderProvider.getOfflinePaymentData().isEmpty) {
                        showCustomSnackBar(
                            getTranslated('input_your_data_properly', context),
                            isError: true);
                        ...
                        print(
                            "dates is ::: ${orderProvider.deliveryDates}and ${orderProvider.selectedDateIndex}");
                        PlaceOrderBody placeOrderBody = PlaceOrderBody(
                          ...
                          branchId: configModel!
                              .branches![orderProvider.branchIndex].id,
                          ...
                          deliveryDate: orderProvider
                              .deliveryDates[orderProvider.selectedDateIndex],
                          couponDiscountTitle: '',
                        ...
                        if (placeOrderBody.paymentMethod == 'wallet_payment' ||
                          ...
                          String selectedUrl;
                          if (cartList.first.product!.isSubscriptionProduct ==
                              1) {
                            String url =
                                "customer_id=${profileProvider.userInfoModel?.id ?? authProvider.getGuestId()}&&is_guest=${authProvider.getGuestId() != null ? '1' : '0'}"
                                "&&callback=${AppConstants.baseUrl}${RouteHelper.orderSuccessful}&&order_amount=${(checkOutData.amount! + (checkOutData.deliveryCharge ?? 0)).toStringAsFixed(2)}&&plan_id=${cartList.first.variation!.type == "weekly" ? cartList.first.product!.paypalWeeklyPlanId : cartList.first.variation!.type == "bi-weekly" ? cartList.first.product!.paypalBiWeeklyPlanId : cartList.first.product!.paypalMonthlyPlanId}&&paypal_product_id=${cartList.first.product!.paypalProductId}&&quantity=${cartList.first.quantity}&&deliveryCharge=${checkOutData.deliveryCharge ?? 0}&&start_date=${orderProvider.deliveryDates[orderProvider.selectDateSlot].substring(0,10)}T23:00:00Z&&delivery_address_id=${Provider.of<LocationProvider>(context, listen: false).addressList![orderProvider.addressIndex].id}";

                            String webUrl =
                                "customer_id=${profileProvider.userInfoModel?.id ?? authProvider.getGuestId()}&&is_guest=${authProvider.getGuestId() != null ? '1' : '0'}"
                                "&&callback=$protocol//$hostname${kDebugMode ? ':$port' : ''}${'${RouteHelper.orderWebPayment}/get_way'}&&order_amount=${(checkOutData.amount! + (checkOutData.deliveryCharge ?? 0)).toStringAsFixed(2)}&&plan_id=${cartList.first.variation!.type == "weekly" ? cartList.first.product!.paypalWeeklyPlanId : cartList.first.variation!.type == "bi-weekly" ? cartList.first.product!.paypalBiWeeklyPlanId : cartList.first.product!.paypalMonthlyPlanId}&&paypal_product_id=${cartList.first.product!.paypalProductId}&&quantity=${cartList.first.quantity}&&deliveryCharge=${checkOutData.deliveryCharge ?? 0}&&start_date=${orderProvider.deliveryDates[orderProvider.selectDateSlot].substring(0,10)}T23:00:00Z&&delivery_address_id=${Provider.of<LocationProvider>(context, listen: false).addressList![orderProvider.addressIndex].id}&&status=";
                            ...
                            selectedUrl =
                                '${AppConstants.baseUrl}/payment-mobile/subscription?token=$tokenUrl&&payment_method=${orderProvider.selectedPaymentMethod?.getWay}&&payment_platform=${kIsWeb ? 'web' : 'app'}&&is_partial=${orderProvider.partialAmount == null ? '0' : '1'}';
