                        ...
                        if (placeOrderBody.paymentMethod == 'wallet_payment' ||
                          ...
                          if (cartList.first.product!.isSubscriptionProduct ==
                            ...
                            String url =
                                ...
                                "&&callback=${AppConstants.baseUrl}${RouteHelper.orderSuccessful}&&order_amount=${(checkOutData.amount! + (checkOutData.deliveryCharge ?? 0)).toStringAsFixed(2)}&&plan_id=${cartList.first.variation!.type == "weekly" ? cartList.first.product!.paypalWeeklyPlanId : cartList.first.variation!.type == "bi-weekly" ? cartList.first.product!.paypalBiWeeklyPlanId : cartList.first.product!.paypalMonthlyPlanId}&&paypal_product_id=${cartList.first.product!.paypalProductId}&&quantity=${cartList.first.quantity}&&deliveryCharge=${checkOutData.deliveryCharge ?? 0}&&start_date=${orderProvider.deliveryDates[orderProvider.selectDateSlot].substring(0,10)}T23:00:00Z";

                            String webUrl =
                                "customer_id=${profileProvider.userInfoModel?.id ?? authProvider.getGuestId()}&&is_guest=${authProvider.getGuestId() != null ? '1' : '0'}"
                                "&&callback=$protocol//$hostname${kDebugMode ? ':$port' : ''}${'${RouteHelper.orderWebPayment}/get_way'}&&order_amount=${(checkOutData.amount! + (checkOutData.deliveryCharge ?? 0)).toStringAsFixed(2)}&&plan_id=${cartList.first.variation!.type == "weekly" ? cartList.first.product!.paypalWeeklyPlanId : cartList.first.variation!.type == "bi-weekly" ? cartList.first.product!.paypalBiWeeklyPlanId : cartList.first.product!.paypalMonthlyPlanId}&&paypal_product_id=${cartList.first.product!.paypalProductId}&&quantity=${cartList.first.quantity}&&deliveryCharge=${checkOutData.deliveryCharge ?? 0}&&start_date=${orderProvider.deliveryDates[orderProvider.selectDateSlot].substring(0,10)}T23:00:00Z&&status=";
